...
export default function AuthorizedMenu() {
  ...
  return (
    <Menu as='div' className='relative inline-block text-left'>
      <Menu.Button className='flex items-center gap-4 focus:outline-none'>
        <Avatar
          src={
      ...
      >
        <Menu.Items
          as='ul'
          className='absolute right-0 mt-1 w-48 origin-top-right rounded bg-white shadow-md focus:outline-none'
        >
          <Menu.Item key={data?.email}>
            <li
              className='flex w-full flex-col space-y-1 rounded-t
             bg-[#0D9965] px-4 py-3 text-sm text-white'
            >
