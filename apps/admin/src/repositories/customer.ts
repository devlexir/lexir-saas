...
class Customer extends Base<CreateCustomer, UpdateUser> {
  // JPCARVALHO 03-05-2022
  create = async (url: string, variables: CustomerInput) => {
    return this.http<CustomerInput>(url, "post", variables);
  };

  newShippingAddress = async (url: string, variables: CustomerInput) => {
    return this.http<CustomerInput>(url, "post", variables);
  };
