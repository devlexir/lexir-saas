...
export interface ButtonProps extends ButtonHTMLAttributes<HTMLButtonElement> {
  className?: string;
  variant?: 'normal' | 'outline' | 'custom' | 'customOutline' | 'saasPrimary';
...
const classes = {
  ...
  customOutline:
    'border border-fill-two bg-transparent text-[#2d2d2d] hover:text-[#636363]',
  saasPrimary:
    'border border-border-400 bg-[#2d2d2d] text-white hover:text-[#CCC]',
  loading:
  ...
  disabledCustom:
    'border border-transparent text-muted cursor-not-allowed bg-[#3e3e3e]',
  ...
  (props, ref) => {
    ...
    const classesName = cn(
      classes.root,
      {
        ...
        [classes.customOutline]: !disabled && variant === 'customOutline',
        [classes.disabledOutline]: disabled && variant === 'outline',
        [classes.custom]: !disabled && variant === 'custom',
        [classes.disabledCustom]: disabled && variant === 'custom',
        [classes.saasPrimary]: !disabled && variant === 'saasPrimary',
